 ...
 * @author Jonas
 */
public class SoundPresenter extends JPanel implements SoundWatcher{
    ...
    public void paint(Graphics g){
        ...
        int bitDepth = (int)SoundCapturer.AudioFormat.getFrameRate()*SoundCapturer.AudioFormat.getSampleSizeInBits();
...
//        System.out.println("Max="+max + " Bitdepth=" + bitDepth);
        for(int i = 0; i < framelength; i++){
            if(i%(framelength/16) == 0){
                ...
                g2.drawLine(width*i/framelength,this.getHeight() , width*i/framelength, 0);
            }
            g2.setColor(Color.gray);
            int yVal = (int)(this.getHeight()*(1.0 - (double)bitDepth/(double)newMax));
            ...
            if(data != null){
                ...
                g2.setColor(new Color(
                        (int)Math.round(255.0*((double)tmp1/(double)newMax)),
                        (int)Math.round(128.0+127.0*((double)tmp1/(double)newMax)),
                        (int)Math.round(255.0-255.0*((double)tmp1/(double)newMax))));
...
//                int sideLength = (int)Math.ceil((double)9*(double)tmp1/(double)max + 3*Math.random());
//                g2.fillRect(
...
//        System.out.println("Presenter (Paint): " + Thread.currentThread().getName());
    }
    static void presentData(DataFrame f) {
    	// Todo: Remake this class to paint a transition from last state to current state.
