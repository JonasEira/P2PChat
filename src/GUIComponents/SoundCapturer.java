 ...
 * @author Jonas
 */
class SoundCapturer implements Runnable {
	...
	public SoundCapturer(TargetDataLine input, AudioFormat audioFormat, int fftLength) {
		receivedBuffer = new byte[0];
		_swlist = new ArrayList<SoundWatcher>();
		running = false;
		_input = input;
		_frameSize = fftLength;
	...
	public void run() {
		...
		if (_input.isOpen()) {
			...
			// int frameSize =
			// (int)Math.pow(2,Math.floor(Math.log(AudioFormat.getFrameRate())/Math.log(2)))/16;
			...
			running = true;
			// System.out.println("Starting Line read - BufferSize: " +
			// _input.getBufferSize() + ", input level: "
			// + _input.getLevel() + ", BitDepth: " +
			// _input.getFormat().getSampleSizeInBits());
			while (isRunning()) {

				int numRead = _input.read(receivedBuffer, 0,
